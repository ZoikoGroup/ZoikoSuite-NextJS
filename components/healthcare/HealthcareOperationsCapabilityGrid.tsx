"use client";

import React from "react";

const capabilities = [
  {
    title: "Finance & spend governance",
    description:
      "Capital approval, purchased-services spend, budget authority and commitment evidence across entities and facilities.",
    footer: "Not revenue cycle, claims or patient billing.",
  },
  {
    title: "Vendor & third-party governance",
    description:
      "Diligence, agreements, renewals, dependency exposure and data-access arrangements with named owners.",
    footer: "Not clinical vendor qualification or device validation.",
  },
  {
    title: "Contract & commercial governance",
    description:
      "Signatory authority, obligations, notice windows and renewal governance across the agreement estate.",
    footer: "Not payer contracting outcomes or rate determination.",
  },
  {
    title: "Workforce administrative governance",
    description:
      "Employment, policy acknowledgement, delegated authority and administrative credential tracking.",
    footer: "Not clinical privileging, competency or credentialing decisions.",
  },
  {
    title: "Obligation & policy management",
    description:
      "Regulatory, accreditation, contractual and internal obligations with basis, owner, due date and evidence.",
    footer: "Not a compliance determination or accreditation outcome.",
  },
  {
    title: "Evidence & audit readiness",
    description:
      "Decision basis, workflow history, document lineage and access events preserved as work happens.",
    footer: "Not the legal medical record or clinical audit.",
  },
  {
    title: "Entity & facility context",
    description:
      "Which entity, facility, service line or operating unit owns an action, and which rules apply to it.",
    footer: "Not a licensure register or facility accreditation record.",
  },
  {
    title: "Change & exception governance",
    description:
      "Material change with authority in the path, and exceptions carrying owner, compensating control and expiry.",
    footer: "Not clinical protocol or care-pathway change.",
  },
  {
    title: "Governed intelligence",
    description:
      "Anomaly detection, prioritization, extraction and decision support within human and policy boundaries.",
    footer: "Not clinical decision support or any patient-affecting decision.",
  },
];

export default function HealthcareOperationsCapabilityGrid() {
  return (
    <section className="w-full bg-[#F7F5F0] text-[#08222F] py-20 px-6 lg:px-12 font-sans">
      <div className="max-w-6xl mx-auto">
        {/* Header Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 items-end">
          <div className="lg:col-span-7 flex flex-col items-start">
            {/* Eyebrow Tag */}
            <div className="flex items-center gap-2 mb-3">
              <span
                className="w-4 h-[1px]"
                style={{ backgroundColor: "#C59B3F" }}
              ></span>
              <span
                className="text-[12px] font-semibold tracking-widest uppercase font-mono"
                style={{ color: "#C59B3F" }}
              >
                HEALTHCARE OPERATIONS CAPABILITY GRID
              </span>
            </div>

            {/* Main Heading */}
            <h2 className="text-3xl sm:text-4xl lg:text-[44px] font-bold tracking-tight leading-[1.1]">
              Nine enterprise jobs, each with its clinical boundary
            </h2>
          </div>

          <div className="lg:col-span-5">
            <p className="text-[13px] lg:text-[14px] text-gray-600 leading-relaxed">
              Validated platform domains translated into healthcare
              enterprise-operating work. Each names what it explicitly does not
              do.
            </p>
          </div>
        </div>

        {/* Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {capabilities.map((item, index) => (
            <div
              key={index}
              className="bg-white rounded-xl p-5 border border-[#0F476A] border-t-[3px] border-t-[#0F476A] shadow-sm flex flex-col justify-between"
            >
              <div>
                <h3 className="text-[14px] font-bold text-[#08222F] mb-2 leading-snug">
                  {item.title}
                </h3>
                <p className="text-[12px] text-gray-600 leading-relaxed mb-4">
                  {item.description}
                </p>
              </div>

              <div className="border-t border-dashed border-gray-200 pt-3">
                <p className="text-[12px] text-[#A63A3A] font-medium leading-relaxed">
                  {item.footer}
                </p>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
